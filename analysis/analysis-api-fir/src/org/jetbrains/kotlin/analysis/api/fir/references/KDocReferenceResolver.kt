...
internal object KDocReferenceResolver {
     ...
     * declarations that are located inside the [contextElement]. Such declarations include: `this` references, the name of
     * the context declaration, type/value parameters of this context declaration, etc.
     *
     ...
     * This list is a combination of lexical scopes for this position and scopes acquired from [org.jetbrains.kotlin.analysis.low.level.api.fir.util.ContextCollector].
    ...
    private fun KaSession.handleContextDeclarations(
        ...
        if (fqName.isOneSegmentFQN() && !containedTagSectionIfSubject.isThrowableExpected()) {
            /**
             * Extension receivers should not be used as tag section subjects,
             * as there is a dedicated `@this` tag for such cases.
             */
            if (containedTagSectionIfSubject != null && shortName.asString() == "this") {
                return emptyList()
            }
            ...
            if (containedTagSectionIfSubject == KDocKnownTag.PROPERTY) {
                ...
                val propertySymbol = containingClass.classSymbol?.combinedMemberScope?.callables?.firstOrNull { callable ->
    ...
    private fun KaSession.getVisibleScopes(
        ...
        val lexicalScopes = getLexicalScopesForPosition(position)
        ...
        return localScopeContextScopes + lexicalScopes + globalScopeContextScopes
    }
    ...
    private fun KaSession.findSymbolsInScopes(
        ...
        allScopesPossiblyContainingName.forEach { currentScope ->
            ...
            val classifiers = currentScope.classifiers(shortName).toSet()

            if (containedTagSectionIfSubject.isThrowableExpected()) {
                classifiers.getThrowableSymbols().ifNotEmpty { return this.toResolveResults() }

                return@forEach
            } else {
                classifiers.ifNotEmpty { return this.toResolveResults() }
            }

            val allCallables = currentScope.callables(shortName).toSet()
    ...
    /**
     * When the link is a subject of [KDocKnownTag.EXCEPTION] or [KDocKnownTag.PROPERTY],
     ...
     *
     * [shouldCollectPropertyParameters] defines the handling of primary constructor properties.
     * When set to `true`, only collects parameter symbols for these constructor properties.
     * Otherwise, collects the corresponding property symbols.
     */
                ...
                for (valueParameter in declaration.valueParameters) {
                    ...
                    when (declaration) {
                        is KtPrimaryConstructor -> {
                            if (valueParameter.isPropertyParameter() && !shouldCollectPropertyParameters) {
    ...
    private fun KaSession.getTypeQualifiedExtensions(
        ...
        val possibleReceivers =
            scopesContainingPossibleReceivers.flatMap { it.classifiers(receiverTypeName.shortName()) }
                .filterIsInstance<KaClassLikeSymbol>().toSet()
        val possibleExtensionsByScope =
            visibleScopes.map { scope -> scope.callables(extensionName).filter { it.isExtension }.toSet() }
