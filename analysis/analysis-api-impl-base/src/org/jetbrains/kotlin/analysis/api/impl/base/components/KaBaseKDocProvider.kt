...
import org.jetbrains.kotlin.analysis.api.components.KaKDocComment
...
internal class KaKDocCommentImpl(
...
abstract class KaBaseKDocProvider<T : KaSession> : KaBaseSessionComponent<T>(), KaKDocProvider {
    ...
    override fun KaSymbol.findKDoc(): KaKDocComment? = with(analysisSession) {
        ...
        if (this@findKDoc is KaDeclarationSymbol) {
    ...
    private fun KtElement.lookupOwnedKDoc(): KaKDocComment? {
            ...
            if (kdoc != null) {
                if (this is KtConstructor<*>) {
                    ...
                    if (constructorSection != null) {
                        ...
                        return KaKDocCommentImpl(constructorSection, paramSections)
                    }
                }
                return KaKDocCommentImpl(kdoc.getDefaultSection(), kdoc.getAllSections())
            }
        }
        return null
    }
    ...
    private fun KtDeclaration.lookupKDocInParent(): KaKDocComment? {
        ...
        return primaryContent?.let {
            ...
            KaKDocCommentImpl(it, additionalSections = emptyList())
        }
    }
}
