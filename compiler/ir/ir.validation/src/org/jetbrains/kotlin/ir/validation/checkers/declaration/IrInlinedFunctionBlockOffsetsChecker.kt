...
object IrInlinedFunctionBlockOffsetsChecker : IrElementChecker<IrInlinedFunctionBlock>(IrInlinedFunctionBlock::class) {
    override fun check(
    ...
    ) = ParentChainUpdater.runInNewContext(context, element) {
        ...
        element.statements.forEach { statement ->
            if ((statement.startOffset in 0..<element.inlinedFunctionStartOffset) ||
                ...
                context.error(
                    statement,
                    "Statement in IrInlinedFunctionBlock has offsets range ${statement.dumpOffsets()}" +
                            " that exceeds the offsets range ${element.dumpOffsets()} of the block"
                )
            }
        }
    }
