    ...
    internal val distribution = run {
        val overridenProperties = mutableMapOf<String, String>().apply {
            if (macabi) {
                ...
                when (target) {
                    "ios_x64" -> {
                        put("targetTriple.ios_x64", "x86_64-apple-ios-macabi")
                        put("targetSysRoot.ios_x64", "\$targetSysRoot.macos_x64")
                    }
                    "ios_simulator_arm64" -> {
                        // macabi implies usage of macOS sysroot.
                        put("targetTriple.ios_simulator_arm64", "arm64-apple-ios-macabi")
                        put("targetSysRoot.ios_simulator_arm64", "\$targetSysRoot.macos_arm64")
                    }
                }
            }
