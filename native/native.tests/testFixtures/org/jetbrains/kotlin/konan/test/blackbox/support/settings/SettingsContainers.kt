    ...
    get() {
        val propertyOverrides = buildMap {
            ...
            if (macabi) {
                ...
                if (target !in setOf(KonanTarget.IOS_X64, KonanTarget.IOS_SIMULATOR_ARM64)) {
                    error("Unsupported target for macabi: $target")
                }
                val arch = when (target.architecture) {
                    Architecture.X64 -> "x86_64"
                    Architecture.ARM64 -> "arm64"
                    else -> error("Unsupported architecture for macabi: $target")
                }
