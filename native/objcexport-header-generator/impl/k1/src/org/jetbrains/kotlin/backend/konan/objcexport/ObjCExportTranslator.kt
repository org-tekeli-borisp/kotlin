...
class ObjCExportTranslatorImpl(
    ...
    internal fun mapReferenceTypeIgnoringNullability(kotlinType: KotlinType, objCExportScope: ObjCExportScope): ObjCNonNullReferenceType {
        ...
        if (KotlinBuiltIns.isAny(classDescriptor) || classDescriptor.classId in mapper.hiddenTypes || classDescriptor.isInlined()) {
            return ObjCIdType
        }
...
internal fun List<PackageFragmentDescriptor>.makePackagesOrderStable() =
 ...
 */
internal fun Map<SourceFile, MutableList<CallableMemberDescriptor>>.makeFilesOrderStable() =
 ...
 */
internal fun Map<ClassDescriptor, MutableList<CallableMemberDescriptor>>.makeCategoriesOrderStable() =
    this.entries.sortedBy { it.key.classId.toString() }

internal fun List<ClassDescriptor>.makeClassesOrderStable() =
